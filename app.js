...
(async () => {
  ...
  addForm.addEventListener('submit', async (e) => {
    ...
    if (!name) {
      ...
      void habitInput.offsetWidth; // reflow to restart animation
      habitInput.classList.add('input-shake');
      let errMsg = habitInput.parentElement.querySelector('.habit-input-error');
      if (!errMsg) {
        errMsg = document.createElement('span');
        errMsg.className = 'habit-input-error';
        errMsg.textContent = 'Please enter a habit name';
        habitInput.parentElement.appendChild(errMsg);
      }
      habitInput.focus();
      // Remove error when user starts typing
      const clearErr = () => {
        errMsg.remove();
        habitInput.removeEventListener('input', clearErr);
      };
      habitInput.addEventListener('input', clearErr);
      return;
    }
